...
```{r Code Hidden but Plot Shows, echo = FALSE}
ggplot(data = mpg, 
...
```{r Warning Message Removed Code, message = FALSE}
ggplot(data = mpg, 
       mapping = aes(y = manufacturer, x = hwy)) + 
  geom_density_ridges() +
  labs(x = "",
...
```{r Both Plots Shown Side-by-Side, fig.show = "hold", out.width = "50%"}
ggplot(data = mpg, 
       mapping = aes(y = manufacturer, x = hwy)) + 
  geom_boxplot() +
  labs(x = "",
...
```{r Code Shows but Output Does Not, eval = FALSE}
2 + 2
```
...
```{r File Still Knits Even Through Error, error = TRUE}
2 + a
```
